...
![image](https://github.com/user-attachments/assets/adc47940-2846-4193-a459-f86c3dd086fc)
...
### Основной сценарий:
 - Administrator вызывает метод addRule(serviceId, ruleData) у контроллера AdminController.
 - Контроллер передает запрос в сервис AdminService, вызывая метод addRule(serviceId, ruleData).
 - Сервис запрашивает информацию о сервисе через ServiceRepository, используя метод findById(serviceId).
 - Repository выполняет SQL-запрос в БД: SELECT * FROM Service WHERE id = serviceId.
 - База данных возвращает информацию о сервисе.
 - Repository возвращает сервис обратно в сервис.
 - Сервис проверяет, является ли сервис валидным:
 - Если сервис не найден (ServiceNotFoundException), контроллер возвращает сообщение об ошибке пользователю.
 - Если сервис архивный (ArchivedServiceException), контроллер сообщает пользователю об ошибке.
 - Если сервис валиден, сервис переходит к валидации данных для нового правила.
 - Сервис валидирует данные правила (validateRuleData(ruleData)).
 - Если данные корректны, сервис сохраняет новое правило в репозитории (RuleRepository).
 - Repository выполняет SQL-запрос в БД: INSERT INTO Rule.
 - База данных сохраняет новое правило и возвращает сохраненную запись.
 - Repository возвращает сохраненное правило в сервис.
 - Сервис возвращает результат в контроллер.
 - Контроллер передает сообщение об успешном добавлении правила пользователю.
 - Если данные некорректны (InvalidRuleDataException), контроллер сообщает об ошибке.
